// components/nfts/BuyConfirmModal.tsx
...
const BuyConfirmModal = ({
  ...
  if (!nftItem.listing || !nftItem.listing.active) return null;
        ...
        <div className="mb-6">
          <div className="flex items-center mb-4">
            <div className="w-20 h-20 relative overflow-hidden rounded-lg mr-4 border border-blue-700/50">
              <Image
                src={imageUrl}
                alt={nftItem.metadata?.name || `NFT #${tokenId}`}
                fill
                className="object-cover"
              />
            </div>
            <div>
              <p className="text-blue-300 text-sm">You are about to purchase</p>
              <p className="text-white font-bold text-lg">
                {nftItem.metadata?.name || `NFT #${tokenId}`}
              </p>
              {isERC1155 && rarityName && (
                <p className="text-blue-300 text-xs mt-1">
                  {rarityName} • Token #{tokenId}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-blue-800/20 border border-blue-700/30 rounded-lg p-3 text-blue-200 text-sm mb-6">
            <div className="flex items-start">
              ...
              <span>
                This action cannot be undone. Once confirmed, your purchase will
